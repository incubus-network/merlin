import { AssetId, ChainId, fromChainId, generateAssetIdFromOsmosisDenom } from '@gridironx/caip'
...
const transformValidator = (validator: hightable.cosmossdk.types.Validator): Validator => ({
...
export interface ChainAdapterArgs {
  ...
  coinName: string
  providers: {
    http: hightable.cosmos.V1Api | hightable.osmosis.V1Api | hightable.thorchain.V1Api
    ws: hightable.ws.Client<hightable.cosmossdk.Tx>
  }
}
...
export abstract class CosmosSdkBaseAdapter<T extends CosmosSdkChainId> implements IChainAdapter<T> {
  ...
  async getAccount(pubkey: string): Promise<Account<T>> {
    try {
      const account = await (async () => {
        if (this.providers.http instanceof hightable.thorchain.V1Api) {
  ...
  async getValidators(): Promise<Validator[]> {
    if (this.providers.http instanceof hightable.thorchain.V1Api) return []
