 ...
 * @group unit
 */
import { ASSET_REFERENCE, fromChainId, optimismAssetId, optimismChainId } from '@gridironx/caip'
...
const makeChainAdapterArgs = (overrideArgs?: {
  providers?: { http: hightable.optimism.V1Api }
  chainId?: EvmChainId
}): ChainAdapterArgs<hightable.optimism.V1Api> =>
  merge(
    {
      providers: {
        http: {} as hightable.optimism.V1Api,
        ws: {} as hightable.ws.Client<hightable.optimism.Tx>,
      },
      rpcUrl: '',
    },
    overrideArgs,
...
describe('OptimismChainAdapter', () => {
  ...
  describe('getFeeData', () => {
    it('should return current network fees', async () => {
      ...
      } as unknown as hightable.optimism.V1Api
