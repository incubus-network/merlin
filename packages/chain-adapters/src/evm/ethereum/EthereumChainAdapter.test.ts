 ...
 * @group unit
 */
import { ASSET_REFERENCE, CHAIN_REFERENCE, ethAssetId, ethChainId } from '@gridironx/caip'
...
describe('EthereumChainAdapter', () => {
  ...
  const makeChainAdapterArgs = (overrideArgs?: {
    providers?: { http: hightable.ethereum.V1Api }
    chainId?: EvmChainId
  }): ChainAdapterArgs<hightable.ethereum.V1Api> =>
    merge(
      {
        providers: {
          http: {} as unknown as hightable.ethereum.V1Api,
          ws: {} as hightable.ws.Client<hightable.ethereum.Tx>,
        },
        rpcUrl: '',
      },
      overrideArgs,
  ...
  describe('getFeeData', () => {
    it('should return current network fees', async () => {
      ...
      } as unknown as hightable.ethereum.V1Api
