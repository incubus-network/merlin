import { AssetId, ChainId } from '@gridironx/caip'
...
export interface ChainAdapterArgs {
  ...
  providers: {
    http:
      | hightable.bitcoin.V1Api
      | hightable.bitcoincash.V1Api
      | hightable.dogecoin.V1Api
      | hightable.litecoin.V1Api
    ws: hightable.ws.Client<hightable.utxo.types.Tx>
  }
}
...
export abstract class UtxoBaseAdapter<T extends UtxoChainId> implements IChainAdapter<T> {
  ...
  protected parser: hightable.utxo.BaseTransactionParser<hightable.utxo.types.Tx>
  ...
  async getTxHistory(input: TxHistoryInput): Promise<TxHistoryResponse> {
    ...
    const getAddresses = (tx: hightable.utxo.types.Tx): string[] => {
