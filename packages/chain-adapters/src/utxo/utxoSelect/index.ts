...
type SanitizedUTXO = Omit<hightable.bitcoin.Utxo, 'value'> & { value: number }
...
export const utxoSelect = (input: UTXOSelectInput) => {
  ...
  const result = (() => {
    if (input.sendMax) {
      ...
      return split<hightable.bitcoin.Utxo>(utxos, outputs, Number(input.satoshiPerByte))
    }

    const outputs = [{ value: Number(input.value), address: input.to }, ...extraOutput]
    return coinSelect<hightable.bitcoin.Utxo>(utxos, outputs, Number(input.satoshiPerByte))
  })()
