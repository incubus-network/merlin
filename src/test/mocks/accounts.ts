...
import { type Account } from '@gridironx/chain-adapters'
...
export const mockEthAndBtcAccounts = ({
  ...
  const ethAccount = merge(
    mockEthAccount({
      chainSpecific: {
        nonce: 1,
        tokens: [
          mockEthToken({ balance: '3000000000000000000', assetId: merlinAssetId }),
  ...
  const ethAccount2 = merge(
    mockEthAccount({
      ...
      chainSpecific: {
        nonce: 1,
        tokens: [mockEthToken({ balance: '2000000000000000000', assetId: merlinAssetId })],
      },
    }),
    ethAccount2Obj,
  )
