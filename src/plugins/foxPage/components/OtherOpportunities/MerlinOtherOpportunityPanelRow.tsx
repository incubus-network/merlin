...
import { ASSET_NAMESPACE, ethChainId, fromAssetId } from '@gridironx/caip'
...
import type { ExternalOpportunity } from '../../MerlinCommon'

type MerlinOtherOpportunityPanelRowProps = {
  opportunity: ExternalOpportunity
}

export const MerlinOtherOpportunityPanelRow: React.FC<MerlinOtherOpportunityPanelRowProps> = ({
  opportunity,
}) => {
  const {
  ...
  const opportunityButtonTranslation = useMemo(() => {
    if (opportunity.link) return 'plugins.merlinPage.getStarted'
    if (isDemoWallet || !wallet || !supportsETH(wallet)) return 'common.connectWallet'

    return hasActivePosition ? 'plugins.merlinPage.manage' : 'plugins.merlinPage.getStarted'
