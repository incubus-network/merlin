...
import { ethChainId } from '@gridironx/caip'
...
export const MainOpportunity = ({
  ...
  const hasActiveStaking = bnOrZero(merlinEarnOpportunityData?.stakedAmountCryptoBaseUnit).gt(0)

  const opportunityButtonTranslation = useMemo(() => {
    if (isDemoWallet || !wallet || !supportsETH(wallet)) return 'common.connectWallet'
    if (hasActiveStaking) return 'plugins.merlinPage.manage'
    return 'plugins.merlinPage.getStarted'
  }, [isDemoWallet, wallet, hasActiveStaking])

  const isOpportunityButtonReady = useMemo(
    () => Boolean(isDemoWallet || (wallet && !supportsETH(wallet)) || merlinEarnOpportunityData),
    [isDemoWallet, wallet, merlinEarnOpportunityData],
  )
      ...
      <Card.Header>
        <Flex flexDirection='row' alignItems='center' mb={2}>
          ...
          <Text
            ...
            color='inherit'
            translation={[
              'plugins.merlinPage.mainStakingTitle',
              {
        ...
        <Text translation='plugins.merlinPage.mainStakingDescription' color='gray.500' />
      </Card.Header>
      <Card.Body>
        <Flex
          width='full'
        ...
        >
          ...
          >
            <Text translation='plugins.merlinPage.currentApy' color='gray.500' mb={1} />
          ...
          <Flex
            ...
            alignItems={{ base: 'center', md: 'flex-start' }}
          >
            <Text translation='plugins.merlinPage.tvl' color='gray.500' mb={1} />
          ...
          <Flex
            ...
            alignItems={{ base: 'center', md: 'flex-start' }}
          >
            <Text translation='plugins.merlinPage.balance' color='gray.500' mb={1} />
