...
describe('txHistorySlice:utils', () => {
  describe('getRelatedAssetIds', () => {
    ...
    it('can get related asset ids from merlin send', () => {
      const relatedAssetIds = getRelatedAssetIds(MERLINSend)
      expect(relatedAssetIds.length).toEqual(2)
      expect(relatedAssetIds.includes(merlinAssetId)).toBeTruthy()
      expect(relatedAssetIds.includes(ethAssetId)).toBeTruthy()
    })
