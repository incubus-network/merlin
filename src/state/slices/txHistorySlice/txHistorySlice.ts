...
import type { AccountId, AssetId } from '@gridironx/caip'
import { ASSET_NAMESPACE, ethChainId, fromAccountId, toAssetId } from '@gridironx/caip'
import type { Transaction } from '@gridironx/chain-adapters'
...
import type { RebaseHistory } from 'lib/investor/investor-merlin'
...
export const txHistoryApi = createApi({
  ...
  endpoints: build => ({
    getMerlinRebaseHistoryByAccountId: build.query<RebaseHistory[], RebaseTxHistoryArgs>({
      queryFn: ({ accountId, portfolioAssetIds }, { dispatch }) => {
        const { chainId, account: userAddress } = fromAccountId(accountId)
        // merlin is only on eth mainnet, and [] is a valid return type and won't upsert anything
        if (chainId !== ethChainId) return { data: [] }
        // merlin contract address, note not assetIds
        const merlinTokenContractAddress = (() => {
          const contractAddress = merlinAddresses[0].merlin.toLowerCase()
          if (portfolioAssetIds.some(id => id.includes(contractAddress))) return contractAddress
        })()

        // don't do anything below if we don't have MERLIN as a portfolio AssetId
        if (!merlinTokenContractAddress) return { data: [] }

        // setup merlin api
        const merlinApi = getMerlinApi()

        ;(async () => {
          const rebaseHistoryArgs = { userAddress, tokenContractAddress: merlinTokenContractAddress }
          const data = await merlinApi.getRebaseHistory(rebaseHistoryArgs)
          const assetReference = merlinTokenContractAddress
