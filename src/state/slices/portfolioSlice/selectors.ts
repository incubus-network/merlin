...
import type { AccountId, AssetId, ChainId } from '@gridironx/caip'
import {
  avalancheAssetId,
  bchAssetId,
  bscAssetId,
...
export const selectPortfolioTotalFiatBalanceExcludeEarnDupes = createSelector(
  selectPortfolioFiatBalances,
  (portfolioFiatBalances): string => {
    // ETH/MERLIN LP token and MERLIN are portfolio assets, but they're also part of DeFi opportunities
    ...
    const portfolioEarnAssetIdsDuplicates = [merlinEthLpAssetId, merlinAssetId]
 ...
 * e.g. we may be swapping into a new EVM account that does not necessarily contain MERLIN
 * but can contain it
 */
...
export const selectBalanceChartCryptoBalancesByAccountIdAboveThreshold =
  createDeepEqualOutputSelector(
    ...
    (
      ...
      const merlinEthLpTotalBalancesIncludingDelegations = aggregatedEarnUserStakingOpportunities
        ?.filter(opportunity => merlinEthStakingIds.includes(opportunity.assetId as StakingId))
        ...
        .toFixed()
      totalBalancesIncludingAllDelegationStates[merlinEthLpAssetId] =
        merlinEthLpTotalBalancesIncludingDelegations
