import type { ToAssetIdArgs } from '@gridironx/caip'
import { ethChainId, merlinAssetId, fromAccountId, fromAssetId, toAssetId } from '@gridironx/caip'
import dayjs from 'dayjs'
import { bn, bnOrZero } from 'lib/bignumber/bignumber'
import { merlinApi } from 'state/apis/merlin/merlinApi'
...
export const merlinStakingOpportunitiesMetadataResolver = async ({
  defiType,
  reduxApi,
}: OpportunitiesMetadataResolverInput): Promise<{ data: GetOpportunityMetadataOutput }> => {
  const allOpportunities = await getMerlinApi().getMerlinOpportunities()

  const merlinApr = await reduxApi.dispatch(merlinApi.endpoints.getMerlinApr.initiate())
  ...
  for (const opportunity of allOpportunities) {
    ...
    const apy = merlinApr.data?.merlinApr ?? '0'
...
export const merlinStakingOpportunitiesUserDataResolver = async ({
  accountId,
  reduxApi,
  opportunityIds,
  ...
  const merlinInvestor = getMerlinApi()

  for (const stakingOpportunityId of opportunityIds) {
    const balanceFilter = { accountId, assetId: merlinAssetId }
    const balance = selectPortfolioCryptoBalanceBaseUnitByFilter(state, balanceFilter)

    const asset = selectAssetById(state, merlinAssetId)
    if (!asset) continue
    ...
    const opportunities = await merlinInvestor.getMerlinOpportunities()

    // investor-merlin is architected around many MERLIN addresses/opportunity, but akchually there's only one
    if (!opportunities[0]) continue

    const opportunity = opportunities[0]

    // MERLIN is a rebasing token so there aren't rewards in the sense of rewards claim
    ...
    const withdrawInfo = await merlinInvestor.getWithdrawInfo({
...
export const merlinStakingOpportunityIdsResolver = async (): Promise<{
  data: GetOpportunityIdsOutput
}> => {
  const opportunities = await getMerlinApi().getMerlinOpportunities()
