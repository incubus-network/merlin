import { merlinAssetId, fromAccountId, fromAssetId } from '@gridironx/caip'
...
import {
  assertIsMerlinEthStakingContractAddress,
  merlinEthLpAssetId,
  merlinEthPair,
  merlinEthStakingIds,
...
export const ethMerlinStakingMetadataResolver = async ({
  opportunityId,
  defiType,
  reduxApi,
  ...
  const lpAssetPrecision = assets.byId[merlinEthLpAssetId]?.precision ?? 0
  const lpTokenMarketData: MarketData = selectMarketDataById(state, merlinEthLpAssetId)
  ...
  if (bnOrZero(lpTokenPrice).isZero()) {
    throw new Error(`Market data not ready for ${merlinEthLpAssetId}`)
  }

  assertIsMerlinEthStakingContractAddress(contractAddress)
  const merlinFarmingContract = getOrCreateContractByAddress(contractAddress)
  const uniV2LPContract = getOrCreateContractByAddress(ETH_MERLIN_POOL_CONTRACT_ADDRESS)

  // tvl
  const totalSupply = await merlinFarmingContract.totalSupply()
  ...
  // apr
  const merlinRewardRatePerTokenV6 = await rewardRatePerToken(merlinFarmingContract)

  const pair = await fetchUniV2PairData(merlinEthLpAssetId)
  ...
  const merlinReserves = bnOrZero(bnOrZero(reserves[1].toString()).toString())
  ...
  const merlinPoolRatio = merlinReserves.div(lpTotalSupply).toString()
  ...
  const merlinEquivalentPerLPToken = token1PoolReservesEquivalent
    .div(bnOrZero(totalSupplyV2.toString()))
    .times(bn(10).pow(pair.token1.decimals)) // convert to base unit value
    .toString()
  const apy = bnOrZero(makeTotalLpApr(merlinRewardRatePerTokenV6, merlinEquivalentPerLPToken))
    .div(100)
    .toString()

  const timeStamp = await merlinFarmingContract.periodFinish()
  const expired =
  ...
  const data = {
    byId: {
      [opportunityId]: {
        ...
        underlyingAssetId: merlinEthLpAssetId,
        underlyingAssetIds: merlinEthPair,
        underlyingAssetRatiosBaseUnit: [
          toBaseUnit(ethPoolRatio.toString(), assets.byId[merlinEthPair[0]]?.precision ?? 0),
          toBaseUnit(merlinPoolRatio.toString(), assets.byId[merlinEthPair[1]]?.precision ?? 0),
        ] as const,
        expired,
        name: 'Merlin Farming',
        version,
        rewardAssetIds: [merlinAssetId] as const,
...
export const ethMerlinStakingUserDataResolver = async ({
  opportunityId,
  accountId,
  reduxApi,
  ...
  const stakedBalance = await merlinFarmingContract.balanceOf(accountAddress)
  const earned = await merlinFarmingContract.earned(accountAddress)
...
export const ethMerlinStakingOpportunityIdsResolver = (): Promise<{
  data: GetOpportunityIdsOutput
}> => Promise.resolve({ data: [...merlinEthStakingIds] })
