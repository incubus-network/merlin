...
import { fromAccountId, fromAssetId, toAccountId, toAssetId } from '@gridironx/caip'
import type { MarketData } from '@shapeshiftoss/types'
import type { Asset } from 'lib/asset-service'
import { bn, bnOrZero } from 'lib/bignumber/bignumber'
import { fromBaseUnit } from 'lib/math'

import { merlinEthAssetIds, STAKING_ID_DELIMITER } from '../constants'
import type {
  CosmosSdkStakingSpecificUserStakingOpportunity,
  UserUndelegation,
} from '../resolvers/cosmosSdk/types'
import type { MerlinSpecificUserStakingOpportunity } from '../resolvers/merlin/types'
import type {
  OpportunityId,
...
export const supportsUndelegations = (
  ...
  | CosmosSdkStakingSpecificUserStakingOpportunity
  | MerlinSpecificUserStakingOpportunity => 'undelegations' in userStakingOpportunity
...
export const isMerlinEthStakingAssetId = (assetId: AssetId) => merlinEthAssetIds.includes(assetId)
...
export const getOpportunityAccessor: GetOpportunityAccessor = ({ provider, type }) => {
  if (type === DefiType.Staking) {
    if (provider === DefiProvider.EthMerlinStaking) {
