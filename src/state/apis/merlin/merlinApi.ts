...
import { CHAIN_REFERENCE } from '@gridironx/caip'
...
export const merlinApi = createApi({
  ...BASE_RTK_CREATE_API_CONFIG,
  reducerPath: 'merlinApi',
  endpoints: build => ({
    getMerlinApr: build.query<GetMerlinAprOutput, void>({
      queryFn: async () => {
        try {
          ...
          return { data: { merlinApr: tMerlinPool.liquidityProviderApr } }
        } catch (e) {
...
export const { useGetMerlinAprQuery } = merlinApi
