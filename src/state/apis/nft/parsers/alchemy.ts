import type { AssetNamespace, ChainId } from '@gridironx/caip'
import { polygonChainId, toAssetId } from '@gridironx/caip'
import type { TokenType } from '@gridironx/hightable-client/src/evm/ethereum'
...
export const parseAlchemyNftToNftItem = async (
  ...
  // If we have an IPNS gateway metadata URL, it means hightable can get a fresh media URL
  // Which allows us to circumvent Alchemy refresh working for Ethereum only
  // Notes:
  // - We're only able to get fresh meta from hightable for IPNS URLs, not IPFS ones
  ...
  > => {
    try {
      ...
      // Get hightable meta if Polygon node is alive
