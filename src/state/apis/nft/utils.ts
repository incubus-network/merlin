...
import { isEvmChainId } from '@gridironx/chain-adapters'
...
export const updateNftCollection = (
  ...
  if (
    currentItem.assetId === merlinatarAssetId &&
    !draftItem.socialLinks.find(({ key: name }) => name === 'customizeMerlinatar')
  ) {
    draftItem.socialLinks.push({
      key: 'customizeMerlinatar',
      displayName: '',
...
// i.e MERLINatar refresh on customization update
