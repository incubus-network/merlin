import type { AccountId, AssetId, ChainNamespace, ChainReference } from '@gridironx/caip'
import { toChainId } from '@gridironx/caip'
...
export const useRouteAssetId = () => {
  ...
  const assetId = useMemo(() => {
    ...
    const merlinPageRouteAssetId = getMerlinPageRouteAssetId(location.pathname)

    return routeAssetId ?? merlinPageRouteAssetId
  }, [location.pathname])
