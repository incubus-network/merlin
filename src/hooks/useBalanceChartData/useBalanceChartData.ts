...
import { TransferType, TxStatus } from '@gridironx/hightable-client'
...
export const bucketEvents = (
  ...
  return txAndRebaseEvents.reduce((acc, event) => {
    const eventDayJs = dayjs(event.blockTime * 1000) // hightable uses seconds
...
const debugCharts: DebugCharts = ({ assets, calculatedBuckets, timeframe, txs }) => {
  ...
  const firstTxTimestamp = txs[0].blockTime * 1000 // hightable uses seconds
