...
import { ethChainId, merlinAssetId, merlinatarAssetId, fromAccountId } from '@gridironx/caip'
...
export const TransactionsProvider: React.FC<TransactionsProviderProps> = ({ children }) => {
  ...
  const maybeRefetchOpportunities = useCallback(
    ({ chainId, data, transfers, status }: Transaction, accountId: AccountId) => {
      ...
      const shouldRefetchAllOpportunities = !(
        chainId === ethChainId &&
        // We don't parse MERLIN farming Txs with any specific parser, hence we're unable to discriminate by parser type
        // This will refetch opportunities user data on any MERLIN/ MERLIN LP token transfer Tx
        // But this is the best we can do at the moment to be reactive
        transfers.some(
          ({ assetId }) =>
            [merlinAssetId, merlinEthLpAssetId].includes(assetId) ||
  ...
  const maybeRefetchNfts = useCallback(
    ({ transfers, status }: Transaction) => {
      ...
      // Only on MERLINatar for now, we may want to generalize this to all NFTs with isNft(assetId) in the future
      if (transfers.some(({ assetId }) => assetId.includes(merlinatarAssetId))) {
