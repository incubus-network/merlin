...
export type MerlinWithdrawValues = {
...
> = ({ accountId, onAccountIdChange: handleAccountIdChange, onNext }) => {
  ...
  const methods = useForm<MerlinWithdrawValues>({ mode: 'onChange' })
  ...
  const handleContinue = useCallback(
    async (formValues: MerlinWithdrawValues) => {
      if (!(accountAddress && dispatch && rewardId && merlinApi && bip44Params)) return
      ...
      const getWithdrawGasEstimateCryptoBaseUnit = async (withdraw: MerlinWithdrawValues) => {
        if (!accountAddress) return

        try {
          const feeDataEstimate = await merlinApi.estimateWithdrawFees({
            tokenContractAddress: rewardId,
            contractAddress,
    ...
    <FormProvider {...methods}>
      ...
      >
        <WithdrawTypeField
          asset={stakingAsset}
          handlePercentClick={handlePercentClick}
          feePercentage={bnOrZero(state.merlinFeePercentage).toString()}
        />
      </ReusableWithdraw>
    </FormProvider>
  )
