...
export const Confirm: React.FC<StepComponentProps & { accountId?: AccountId | undefined }> = ({
  ...
  const withdrawalFee = useMemo(() => {
    return state?.withdraw.withdrawType === WithdrawType.INSTANT
      ? bnOrZero(
          bn(state?.withdraw.cryptoAmount ?? '0').times(state?.merlinFeePercentage ?? '0'),
        ).toString()
      : '0'
  }, [state?.withdraw.withdrawType, state?.withdraw.cryptoAmount, state?.merlinFeePercentage])
  ...
  const handleConfirm = useCallback(async () => {
    try {
      if (
        ...
        return
      dispatch({ type: MerlinWithdrawActionType.SET_LOADING, payload: true })

      if (!supportsETH(walletState.wallet))
        throw new Error(`handleConfirm: wallet does not support ethereum`)

      const txid = await merlinApi.withdraw({
      ...
      dispatch({ type: MerlinWithdrawActionType.SET_TXID, payload: txid })
      onNext(DefiStep.Status)

      const transactionReceipt = await poll({
        fn: () => merlinApi.getTxReceipt({ txid }),
      ...
      dispatch({ type: MerlinWithdrawActionType.SET_LOADING, payload: false })
    } catch (error) {
      console.error(error)
    }
  }, [
