...
import { ASSET_NAMESPACE, ASSET_REFERENCE, toAssetId } from '@gridironx/caip'
...
export const ClaimConfirm = ({
  ...
  const handleConfirm = useCallback(async () => {
    if (!(walletState.wallet && contractAddress && userAddress && merlinApi && bip44Params)) return
    setLoading(true)
    try {
      if (!supportsETH(walletState.wallet))
        throw new Error(`handleConfirm: wallet does not support ethereum`)
      const txid = await merlinApi.claimWithdraw({
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (!(walletState.wallet && contractAddress && merlinApi && chainAdapter)) return
        if (!supportsETH(walletState.wallet))
        ...
        const feeDataEstimate = await merlinApi.estimateClaimWithdrawFees({
          claimAddress: userAddress,
          userAddress,
          contractAddress,
          wallet: walletState.wallet,
