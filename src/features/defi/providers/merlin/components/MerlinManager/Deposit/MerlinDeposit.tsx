...
export const MerlinDeposit: React.FC<{
  ...
  const {
    chainId,
    contractAddress: merlinContractAddress,
    assetReference: merlinStakingContractAddress,
    assetNamespace,
  } = query
  // ContractAssetId
  const assetId = toAssetId({ chainId, assetNamespace, assetReference: merlinStakingContractAddress })
  ...
  useEffect(() => {
    ;(async () => {
      try {
        ...
        dispatch({
          ...
          payload: { ...merlinOpportunity, apy: merlinAprData?.merlinApr ?? '' },
        })
      } catch (error) {
    ...
    bip44Params,
    chainAdapterManager,
    merlinContractAddress,
    walletState.wallet,
    merlinAprData?.merlinApr,
    isMerlinAprLoading,
    merlinStakingContractAddress,
  ])
  ...
  }, [accountId, handleAccountIdChange, merlinContractAddress, translate, stakingAsset.symbol])
