...
import { fromAccountId } from '@gridironx/caip'
...
export const Deposit: React.FC<DepositProps> = ({
  ...
  const opportunity = useMemo(() => state?.merlinOpportunity, [state])
  ...
  const handleContinue = useCallback(
    async (formValues: DepositValues) => {
      if (!(state && accountAddress && dispatch && merlinApi)) return

      const getApproveGasEstimate = async () => {
        if (!accountAddress || !assetReference || !merlinApi) return
        try {
          const feeDataEstimate = await merlinApi.estimateApproveFees({
      ...
      const getDepositGasEstimateCryptoBaseUnit = async (deposit: DepositValues) => {
        if (!accountAddress || !assetReference || !merlinApi) return
        try {
          const feeDataEstimate = await merlinApi.estimateDepositFees({
            tokenContractAddress: assetReference,
            contractAddress,
      ...
      dispatch({ type: MerlinDepositActionType.SET_DEPOSIT, payload: formValues })
      dispatch({ type: MerlinDepositActionType.SET_LOADING, payload: true })
      try {
        // Check is approval is required for user address
        const _allowance = await merlinApi.allowance({
          tokenContractAddress: assetReference,
          contractAddress,
          userAddress: accountAddress,
        })
        ...
        if (allowance.gte(formValues.cryptoAmount)) {
          ...
          dispatch({ type: MerlinDepositActionType.SET_LOADING, payload: false })
        } else {
