...
import { ethAssetId, fromAccountId } from '@gridironx/caip'
import type { ethereum } from '@gridironx/chain-adapters'
import {
...
import type { MerlinEthStakingContractAddress } from 'state/slices/opportunitiesSlice/constants'
import { merlinEthLpAssetId } from 'state/slices/opportunitiesSlice/constants'
import { selectAccountNumberByAccountId, selectAssetById } from 'state/slices/selectors'
import { useAppSelector } from 'state/store'

type UseMerlinFarmingOptions = {
  skip?: boolean
}
/**
 * useMerlinFarming hook
 * @param contractAddress farming contract address, since there could be multiple contracts
 * @param skip
 */
export const useMerlinFarming = (
  contractAddress: MerlinEthStakingContractAddress,
  { skip }: UseMerlinFarmingOptions = {},
) => {
  const { farmingAccountId } = useMerlinEth()
  const ethAsset = useAppSelector(state => selectAssetById(state, ethAssetId))
  const lpAsset = useAppSelector(state => selectAssetById(state, merlinEthLpAssetId))

  if (!ethAsset) throw new Error(`Asset not found for AssetId ${ethAssetId}`)
  if (!lpAsset) throw new Error(`Asset not found for AssetId ${merlinEthLpAssetId}`)
  ...
  const merlinFarmingContract = useMemo(
    () => (skip ? null : getOrCreateContractByAddress(contractAddress)),
    [contractAddress, skip],
  )

  const uniV2LPContract = useMemo(
    () => (skip ? null : getOrCreateContractByAddress(ETH_MERLIN_POOL_CONTRACT_ADDRESS)),
    [skip],
  )
  ...
  const stake = useCallback(
    async (lpAmount: string) => {
      try {
        ...
        if (!merlinFarmingContract) return
        if (!wallet) return

        if (!adapter) throw new Error(`no adapter available for ${ethAsset.chainId}`)

        const data = merlinFarmingContract.interface.encodeFunctionData('stake', [
    ...
    async (lpAmount: string, isExiting: boolean) => {
      try {
        ...
        const data = isExiting
          ? merlinFarmingContract.interface.encodeFunctionData('exit')
          : merlinFarmingContract.interface.encodeFunctionData('withdraw', [
              bnOrZero(lpAmount).times(bnOrZero(10).exponentiatedBy(lpAsset.precision)).toFixed(0),
            ])
  ...
  const getClaimFeeData = useCallback(
    async (userAddress: string) => {
      if (!adapter || !merlinFarmingContract || !userAddress) return

      const data = merlinFarmingContract.interface.encodeFunctionData('getReward')
    ...
    [adapter, contractAddress, merlinFarmingContract],
  )
