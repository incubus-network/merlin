...
export const ExpiredWithdraw: React.FC<ExpiredWithdrawProps> = ({
  ...
  const { getUnstakeFeeData, allowance, getApproveFeeData } = useMerlinFarming(contractAddress)
  ...
  const handleContinue = async () => {
    ...
    dispatch({ type: MerlinFarmingWithdrawActionType.SET_LOADING, payload: true })
    dispatch({
    ...
    if (allowanceAmount.gte(amountAvailableCryptoPrecision)) {
      ...
      if (!estimatedGasCrypto) {
        dispatch({ type: MerlinFarmingWithdrawActionType.SET_LOADING, payload: false })
        return
      }
      dispatch({
        type: MerlinFarmingWithdrawActionType.SET_WITHDRAW,
