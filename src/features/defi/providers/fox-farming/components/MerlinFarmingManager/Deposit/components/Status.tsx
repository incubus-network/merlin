...
export const Status: React.FC<StatusProps> = ({ accountId }) => {
  ...
  if (!asset)
    throw new Error(`Asset not found for AssetId ${merlinFarmingOpportunity?.underlyingAssetId}`)
  if (!feeAsset) throw new Error(`Fee asset not found for AssetId ${feeAssetId}`)
  ...
  useEffect(() => {
    if (!merlinFarmingOpportunity) return
  ...
  if (!state || !dispatch || !merlinFarmingOpportunity) return null
      ...
      case 'success':
        return {
          ...
          statusBody: translate('modals.deposit.status.success', {
            opportunity: merlinFarmingOpportunity?.opportunityName,
          }),
          statusBg: 'green.500',
        }
      case 'failed':
  ...
  return (
    <TxStatus
      ...
      pairIcons={merlinFarmingOpportunity?.icons}
    >
      <Summary>
