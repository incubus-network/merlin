...
import { toAssetId } from '@gridironx/caip'
...
export const Deposit: React.FC<DepositProps> = ({
  ...
  const underlyingAssetId = merlinFarmingOpportunity?.underlyingAssetId
  ...
  const { getLpTokenPrice } = useUniV2LiquidityPool({
    accountId: accountId ?? '',
    assetId0: merlinFarmingOpportunity?.underlyingAssetIds[0] ?? '',
    assetId1: merlinFarmingOpportunity?.underlyingAssetIds[1] ?? '',
  ...
  const {
    allowance: merlinFarmingAllowance,
    getStakeFeeData,
    getApproveFeeData,
  } = useMerlinFarming(contractAddress)
  ...
  const handleContinue = useCallback(
    async (formValues: DepositValues) => {
      if (!(state && dispatch && feeAsset && merlinFarmingOpportunity)) return
      ...
      dispatch({ type: MerlinFarmingDepositActionType.SET_DEPOSIT, payload: formValues })
      dispatch({ type: MerlinFarmingDepositActionType.SET_LOADING, payload: true })
      try {
        if (!asset) return
        // Check if approval is required for user address
        const _allowance = await merlinFarmingAllowance()
  ...
  if (!state || !dispatch || !merlinFarmingOpportunity || !asset || !marketData) return null
  ...
  return (
    <ReusableDeposit
      ...
      inputIcons={merlinFarmingOpportunity?.icons}
      apy={String(merlinFarmingOpportunity?.apy)}
