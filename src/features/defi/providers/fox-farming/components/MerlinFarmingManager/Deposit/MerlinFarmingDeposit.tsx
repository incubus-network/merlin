...
type MerlinFarmingDepositProps = {
  onAccountIdChange: AccountDropdownProps['onChange']
  accountId: AccountId | undefined
}
export const MerlinFarmingDeposit: React.FC<MerlinFarmingDepositProps> = ({
  accountId,
  ...
  useEffect(() => {
    ;(() => {
      try {
        if (!(farmingAccountId && contractAddress && merlinFarmingOpportunity)) return
      } catch (error) {
        // TODO: handle client side errors
        console.error(error)
      }
    })()
  }, [farmingAccountId, translate, toast, contractAddress, merlinFarmingOpportunity])
  ...
  if (loading || !marketData || !merlinFarmingOpportunity || !StepConfig) {
    return (
      ...
      <DefiModalContent>
        <DefiModalHeader
          title={translate('modals.deposit.depositInto', {
            opportunity: merlinFarmingOpportunity.name,
          })}
          onBack={handleBack}
        />
