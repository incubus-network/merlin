...
import { MerlinFarmingEmpty } from './MerlinFarmingEmpty'
import { WithdrawCard } from './WithdrawCard'

type MerlinFarmingOverviewProps = {
  accountId?: AccountId | undefined
  onAccountIdChange: AccountDropdownProps['onChange']
}

export const MerlinFarmingOverview: React.FC<MerlinFarmingOverviewProps> = ({
  accountId,
  ...
  const lpAsset = assets[merlinEthLpAssetId]
  ...
  const underlyingAssetsFiatBalance = useMemo(() => {
    ...
    const cryptoAmount = bnOrZero(opportunityData?.stakedAmountCryptoBaseUnit)
      ...
      .toString()
    const merlinEthLpFiatPrice = marketData?.[opportunityData?.underlyingAssetId ?? '']?.price ?? '0'
    return bnOrZero(cryptoAmount).times(merlinEthLpFiatPrice).toString()
  }, [
    marketData,
