...
export const ClaimConfirm = ({ accountId, assetId, amount, onBack }: ClaimConfirmProps) => {
  ...
  const { claimRewards, getClaimFeeData, merlinFarmingContract } = useMerlinFarming(contractAddress)
  ...
  useEffect(() => {
    ;(async () => {
      try {
        if (
          !(walletState.wallet && feeAsset && feeMarketData && merlinFarmingContract && accountAddress)
        )
          return
