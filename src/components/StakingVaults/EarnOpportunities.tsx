...
import { merlinAssetId, merlinAssetId, fromAssetId } from '@gridironx/caip'
import qs from 'qs'
...
export const EarnOpportunities = ({ assetId, accountId }: EarnOpportunitiesProps) => {
  ...
  const { setFarmingAccountId } = useMerlinEth()
      ...
      !asset
        ...
        : lpOpportunities.concat(stakingOpportunities).filter(
            row =>
              ...
              // show merlin opportunity in the merlin asset page
              (row.assetId === merlinAssetId && asset.assetId === merlinAssetId),
          ),
