import type { ChainReference } from '@gridironx/caip'
import { CHAIN_NAMESPACE, CHAIN_REFERENCE, toAssetId } from '@gridironx/caip'
import type { EvmBaseAdapter, FeeDataEstimate } from '@gridironx/chain-adapters'
...
import { merlinAbi } from '../abi/merlin-abi'
import { merlinStakingAbi } from '../abi/merlin-staking-abi'
...
export type ConstructorArgs = {
  ...
  providerUrl: string
  merlinAddresses: MerlinAddressesType
  chainReference?: EthereumChainReference
}

export const transformData = ({ tvl, apy, expired, ...contractData }: MerlinOpportunityInputData) => {
  return {
    type: DefiType.Staking,
    provider: 'ShapeShift',
    version: '1',
    contractAddress: contractData.staking,
    rewardToken: contractData.merlin,
    stakingToken: contractData.merlin,
...
export class MerlinApi {
  ...
  constructor({
    ...
    this.merlinStakingContracts = merlinAddresses.map(
      addresses => new ethers.Contract(addresses.staking, merlinStakingAbi, this.provider),
    )
    this.liquidityReserveContracts = merlinAddresses.map(
      addresses =>
    ...
    this.merlinAddresses = merlinAddresses
  }
  ...
  private getStakingContract(contractAddress: string): ethers.Contract {
    const stakingContract = this.merlinStakingContracts.find(
  ...
  async getMerlinOpportunities() {
    try {
      const opportunities = await Promise.all(
        this.merlinAddresses.map(async addresses => {
          const stakingContract = this.merlinStakingContracts.find(
            item => toLower(item.address) === toLower(addresses.staking),
          )
          try {
            const expired = await stakingContract?.pauseStaking()
            const tvl = await this.tvl({ tokenContractAddress: addresses.merlin })
            const apy = this.apy()
      ...
      throw new Error(`getMerlinOpportunities Error: ${e}`)
    }
  }

  async getMerlinOpportunityByStakingAddress(stakingAddress: string) {
    this.verifyAddresses([stakingAddress])
    const addresses = this.merlinAddresses.find(item => {
  ...
  async tvl(input: TokenAddressInput): Promise<BigNumber> {
    ...
    this.verifyAddresses([tokenContractAddress])
    const contract = new ethers.Contract(tokenContractAddress, merlinAbi, this.provider)
  ...
  async getRebaseHistory(input: BalanceInput) {
    ...
    const merlinContract = new ethers.Contract(tokenContractAddress, merlinAbi, this.provider)
    const fromBlock = 14381454 // genesis rebase

    const rebaseEvents = await (async () => {
      try {
        const filter = merlinContract.filters.LogRebase()
        const events = await merlinContract.queryFilter(filter, fromBlock, 'latest')
    ...
    const transferEvents = await (async () => {
      try {
        const filter = merlinContract.filters.Transfer()
        const events = await merlinContract.queryFilter(filter, fromBlock, 'latest')
        return events
      } catch (e) {
      ...
      events.map(async event => {
        const { preRebaseBalance, postRebaseBalance } = await (async () => {
          try {
            ...
            const postRebaseBalanceResult = await merlinContract.balanceOf(userAddress, {
              blockTag: event.blockNumber,
            })
            const unadjustedPreRebaseBalance = await merlinContract.balanceOf(userAddress, {
    ...
    const actualResults = results.reduce<RebaseHistory[]>((acc, cur) => {
      if (cur.status === 'rejected') {
        console.error('getMerlinRebaseHistory: balanceOf call failed - charts will be wrong')
        return acc
      }
