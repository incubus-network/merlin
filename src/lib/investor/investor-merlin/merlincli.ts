...
import { merlinAddresses } from './constants'
...
const main = async (): Promise<void> => {
  ...
  const ethChainAdapter = new ethereum.ChainAdapter({
    providers: {
      ws: new hightable.ws.Client<hightable.ethereum.Tx>('wss://dev-api.ethereum.incubus.network'),
      http: new hightable.ethereum.V1Api(
        new hightable.ethereum.Configuration({
          basePath: 'https://dev-api.ethereum.incubus.network',
        }),
      ),
    },
  ...
  const merlinContractAddress = merlinAddresses[0].merlin
  const merlinContractAddress = merlinAddresses[0].merlin
  const merlinStakingContractAddress = merlinAddresses[0].staking
  const liquidityReserveContractAddress = merlinAddresses[0].liquidityReserve
  ...
  const circulatingSupply = async () => {
    try {
      const supply = await api.tvl({ tokenContractAddress: merlinContractAddress })
  ...
  const totalSupply = async () => {
    try {
      const supply = await api.totalSupply({ tokenContractAddress: merlinContractAddress })
  ...
  const stakingTokenBalance = async () => {
    try {
      const balance = await api.balance({
        tokenContractAddress: merlinContractAddress,
        userAddress,
      })
    ...
    switch (index) {
      case 0:
        ...
        switch (tokenContract) {
          case 0:
            await approve(merlinContractAddress, merlinStakingContractAddress)
            break
          case 1:
            await approve(merlinContractAddress, merlinStakingContractAddress)
            break
      ...
      case 1:
        ...
        switch (tokenContract) {
          case 0:
            await approve(merlinContractAddress, liquidityReserveContractAddress)
            break
          case 1:
            await approve(merlinContractAddress, liquidityReserveContractAddress)
            break
