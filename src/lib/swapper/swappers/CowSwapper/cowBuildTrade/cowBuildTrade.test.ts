...
import { ETH, MERLIN_MAINNET, USDC_GNOSIS, WBTC, WETH, XDAI } from '../../utils/test-data/assets'
...
jest.mock('@gridironx/chain-adapters')
...
const expectedTradeWethToMerlin: CowTrade<KnownChainIds.EthereumMainnet> = {
  ...
  buyAmountBeforeFeesCryptoBaseUnit: '14707533959600717283163', // 14707 MERLIN
...
const expectedTradeQuoteMerlinToEth: CowTrade<KnownChainIds.EthereumMainnet> = {
...
describe('cowBuildTrade', () => {
  ...
  it('should call cowService with correct parameters, handle the fees and return the correct trade when selling WETH', async () => {
    ...
    expect(maybeBuiltTrade.unwrap()).toEqual(expectedTradeWethToMerlin)
  ...
  it('should call cowService with correct parameters, handle the fees and return the correct trade when buying ETH', async () => {
    ...
    expect(maybeBuiltTrade.unwrap()).toEqual(expectedTradeQuoteMerlinToEth)
    expect(cowService.post).toHaveBeenCalledWith(
      'https://api.cow.fi/mainnet/api/v1/quote/',
      expectedApiInputMerlinToEth,
    )
