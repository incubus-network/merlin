import { ethChainId, gnosisChainId } from '@gridironx/caip'
import type { EvmChainAdapter } from '@gridironx/chain-adapters'
...
const actualChainAdapters = jest.requireActual('@gridironx/chain-adapters')
...
const cowTradeEthToMerlin: CowTrade<KnownChainIds.EthereumMainnet> = {
...
const cowTradeWethToMerlin: CowTrade<KnownChainIds.EthereumMainnet> = {
  rate: '14716.04718939437505555958',
  feeData: {
    protocolFees: {},
    networkFeeCryptoBaseUnit: '14557942658757988',
...
const cowTradeMerlinToEth: CowTrade<KnownChainIds.EthereumMainnet> = {
...
const expectedWethToMerlinOrderToSign: CowSwapOrder = {
...
const expectedMerlinToEthOrderToSign: CowSwapOrder = {
...
describe('cowExecuteTrade', () => {
  ...
  it('should call cowService with correct parameters and return the order uid when selling WETH', async () => {
    ...
    expect(cowService.post).toHaveBeenCalledWith('https://api.cow.fi/mainnet/api/v1/orders/', {
      ...expectedWethToMerlinOrderToSign,
      signingScheme: SIGNING_SCHEME,
      signature: Signature,
      from: expectedWethToMerlinOrderToSign.receiver,
    })
  ...
  it('should call cowService with correct parameters and return the order uid when buying ETH', async () => {
    ...
    expect(cowService.post).toHaveBeenCalledWith('https://api.cow.fi/mainnet/api/v1/orders/', {
      ...expectedMerlinToEthOrderToSign,
      signingScheme: SIGNING_SCHEME,
      signature: Signature,
      from: expectedMerlinToEthOrderToSign.receiver,
    })
