...
jest.mock('state/slices/selectors', () => {
  ...
  return {
    selectAssets: () => ({
      [BTC.assetId]: BTC,
      [ETH.assetId]: ETH,
      [MERLIN_MAINNET.assetId]: MERLIN_MAINNET,
      [MERLIN_GNOSIS.assetId]: MERLIN_GNOSIS,
      [WBTC.assetId]: WBTC,
      [WETH.assetId]: WETH,
      [XDAI.assetId]: XDAI,
    }),
...
describe('CowSwapper', () => {
  ...
  describe('filterBuyAssetsBySellAssetId', () => {
    ...
    it('returns array filtered out of non erc20 tokens when called with a sellable sellAssetId', () => {
      ...
      ).toEqual([ETH.assetId, WBTC.assetId, MERLIN_MAINNET.assetId])
      expect(
        swapper.filterBuyAssetsBySellAssetId({
          assetIds: ASSET_IDS,
          sellAssetId: WBTC.assetId,
        }),
      ).toEqual([ETH.assetId, WETH.assetId, MERLIN_MAINNET.assetId])
      expect(
        swapper.filterBuyAssetsBySellAssetId({
          assetIds: ASSET_IDS,
          sellAssetId: MERLIN_MAINNET.assetId,
        }),
    ...
    it('returns array filtered out of unsupported tokens when called with a sellable sellAssetId', () => {
      ...
      ).toEqual([MERLIN_MAINNET.assetId])
      expect(
        swapper.filterBuyAssetsBySellAssetId({ assetIds, sellAssetId: MERLIN_MAINNET.assetId }),
      ).toEqual([])
    })
  })
