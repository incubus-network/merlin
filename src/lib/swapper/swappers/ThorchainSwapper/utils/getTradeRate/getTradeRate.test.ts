...
import { BTC, ETH, MERLIN_MAINNET, UNSUPPORTED } from '../../../utils/test-data/assets'
...
describe('getTradeRate', () => {
  ...
  it('should calculate a correct rate for trading ETH to MERLIN', async () => {
    ...
    const maybeTradeRate = await getTradeRate({
      sellAsset: ETH,
      buyAssetId: MERLIN_MAINNET.assetId,
      ...
      Promise.resolve(
        Ok({
          data: {
            ...
            fees: {
              affiliate: '0',
              asset: 'ETH.MERLIN-0XC770EEFAD204B5180DF6A14EE197D99D808EE52D',
