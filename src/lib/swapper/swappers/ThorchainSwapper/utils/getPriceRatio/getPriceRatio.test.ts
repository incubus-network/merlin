import type { ChainAdapterManager } from '@gridironx/chain-adapters'
import { Ok } from '@sniptt/monads'
import type { AxiosStatic } from 'axios'
import type Web3 from 'web3'

import type { ThorchainSwapperDeps } from '../../types'
import { ethThornodePool, merlinThornodePool } from '../test-data/responses'
...
describe('getPriceRatio', () => {
  ...
  it('should correctly calculate price ratio of between a given buy and sell asset', async () => {
    const merlinId = 'eip155:1/erc20:0xc770eefad204b5180df6a14ee197d99d808ee52d'
    const ethId = 'eip155:1/slip44:60'
    ;(thorService.get as jest.Mock<unknown>).mockReturnValue(
      Promise.resolve(Ok({ data: [merlinThornodePool, ethThornodePool] })),
    )

    const ratio = await getPriceRatio(deps, { buyAssetId: merlinId, sellAssetId: ethId })
